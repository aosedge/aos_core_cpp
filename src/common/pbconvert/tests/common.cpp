...
{
    aos::InstanceIdent          param {"item-id", "1.2.3", "subject-id", 1, aos::UpdateItemTypeEnum::eComponent};
    ...
    EXPECT_EQ(result.version().c_str(), param.mVersion);
}
...
TEST_F(PBConvertCommon, ConvertInstanceIdentToAos)
{
    ...
    EXPECT_EQ(result.mVersion, param.version().c_str());
}
