...
public:
    ...
    void SetUp() override
    {
        ...
        mCertProviderStub.SetRootCerts({"file://" CERTIFICATES_CM_DIR "/ca.cer"});
...
TEST_F(CMCommunicationTest, ReconnectOnSendFailure)
{
    ...
    // Guarantees sender is stopped and joined on every exit path from this point on, including an ASSERT_*
    // failure below returning early: an unjoined joinable std::thread calls std::terminate() on destruction.
    struct StopSender {
