...
#include <sm/launcher/runtimes/boot/efibootcontroller.hpp>
...
public:
    MOCK_METHOD(
        Error, ReadVariable, (const std::string&, std::vector<uint8_t>&, uint32_t& attributes), (const, override));
    MOCK_METHOD(Error, WriteGlobalGuidVariable, (const std::string&, const std::vector<uint8_t>&), (override));
    ...
    MOCK_METHOD(Error, CreateBootEntry, (const std::string&, int, const std::string&, uint16_t), (override));
};
...
TEST_F(EFIBootControllerTest, EFIBootEntriesAreCreatedOnInit)
{
    ...
    EXPECT_CALL(
        *mEFIVar, WriteGlobalGuidVariable("BootOrder", std::vector<uint8_t> {0x0A, 0x00, 0x0B, 0x00, 0x09, 0x00}))
...
TEST_F(EFIBootControllerTest, SetBootOK)
{
    ...
    EXPECT_CALL(*mEFIVar, WriteGlobalGuidVariable("BootOrder", newBootOrder)).WillOnce(Return(ErrorEnum::eNone));
